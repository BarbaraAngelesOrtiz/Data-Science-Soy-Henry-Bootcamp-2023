...
use henry_m3;
-- 1)Crear un procedimiento que recibe como parametro una fecha y muestre el listado de productos que 
...
-- 2)Crear una función que calcule 
-- el valor nominal de un margen bruto determinado por el usuario a partir del precio de lista de los productos.
...
-- 3) Obtner un listado de productos de IMPRESION y utilizarlo para cálcular el valor nominal 
-- de un margen bruto del 20% de cada uno de los productos.
SELECT 	p.IdProducto, 
		p.Producto,
        p.Precio,
        margenBruto(Precio, 1.2) AS PrecioMargen
FROM producto p 
join tipo_producto tp
	On (p.IdTipoProducto = tp.IdTipoProducto
...
-- listar los productos vendidos desde fact_venta a partir de un "Tipo" que determine el usuario.
...
BEGIN
	...
	FROM fact_venta v
		INNER JOIN dim_cliente c
			ON (v.IdCliente = c.IdCliente
				and c.Rango_Etario collate utf8mb4_spanish_ci like Concat('%', rango_etario, '%'))
	GROUP BY c.Rango_Etario;
END $$
DELIMITER ;
...
-- 7)Crear una variable que se pase como valor para
-- realizar una filtro sobre Rango_etario en una consulta génerica a dim_cliente.
...
-- 8)Utilizar la funcion provista 'UC_Words' para modificar a letra capital 
